...
const bestMovie = title => {
  return title + 'this is the best movie ever!'
} 
...
function bigOrSmall(arr) {
  let (i = 0; i < bigOrSmallArray.length; i++) {
    if (arr[i] > 100) {
      answers.push("Big")
    } else {
      answers.push("Small")
...
function frogScalper(gold) {
  return gold / 3
}

let totalFrogs = frogScalper(1000)
...
/*
  ...
  You would just be able to purchase 1 product. Re-write the function you used in the 
  previous problem (give it the same name, just add a 2 to the end of it) that fixes this bug. 
  Invoke the function and store the returned value to a variable called `totalFrogs2`.
*/

function frogScalper2(gold) {
  if (gold % 3 === 0) {
    return gold / 3
  } else if ((gold - 1) % 3 === 0) {
    return (gold - 1) / 3
...
let totalFrogs2 = frogScalper2(1000)
